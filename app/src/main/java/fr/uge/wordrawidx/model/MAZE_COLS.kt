...
//                restore = { bundle ->
//                    MazeState().apply {
//                        ballPosition = Offset(bundle.getFloat("ballX"), bundle.getFloat("ballY"))
//                        timeLeftSeconds = bundle.getInt("timeLeft", 45)
//                        currentMiniGameState = MiniGameState.valueOf(bundle.getString("miniGameState") ?: MiniGameState.PLAYING.name)
//                    }
//                }
//            )
//    }
...
class MazeState {
    ...
    internal fun updateBallPosition(
        ...
        if (cellWidthPx > 0f && cellHeightPx > 0f) {
            ...
            var ballRectX = Rect(proposedX - ballRadiusPx, originalBallY - ballRadiusPx, proposedX + ballRadiusPx, originalBallY + ballRadiusPx)
            var collisionX = false
            for (wallInGridUnits in walls) {
                val wallPx = Rect(
                    left = wallInGridUnits.left * cellWidthPx,
            ...
            var ballRectY = Rect(proposedX - ballRadiusPx, proposedY - ballRadiusPx, proposedX + ballRadiusPx, proposedY + ballRadiusPx)
            var collisionY = false
            for (wallInGridUnits in walls) {
                val wallPx = Rect(
                    left = wallInGridUnits.left * cellWidthPx,
